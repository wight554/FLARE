...
| `sem_psf_relief_bound` / `_shallow` / `_relief_shallow_rail` (type-P only) | Phase 13 Plan 01: bounded type-P relief never commands the raw clamped max while physically saturated, at rail scales 1.0/0.7/0.5 (the 0.5 scale is the 51bdca8 tripwire — `RELIEF_ON` must still fire) |
| `sem_psf_relief_extreme_stale` (type-P only) | Phase 13 Plan 01 (REVIEW-07): an early deep transient spike, full recovery, then a second shallower starvation later in the same sync window — proves the tracked tension extreme relaxes rail-relatively rather than staying pinned to the first spike forever |
| `sem_psf_relief_ramp_capped` / `_shallow` (type-P only) | Phase 13 Plan 01 Task 2 (D-04): the tension-dwell ramp's target-raise escalation is capped at the same relief bound as the apply-side branch, not just at `max_sps` |
| `sem_psf_mm_trip` / `_shallow` (type-P only) | Phase 13 Plan 02: `SYNC_TENSION_STOP_MM` distance trip fires on a chronic 0.7x underfeed lane (`TENSION_STOP:MM`), at rail scales 1.0/0.7 |
| `sem_psf_mm_trip_disabled` (type-P only) | Phase 13 Plan 02 (D-08/D-28): `SET:SYNC_TENSION_STOP_MM:0` (modeled via `tension_stop_mm_disabled`) suppresses the trip on the identical demand/feed_gain shape as `sem_psf_mm_trip` |
| `sem_psf_trip_unarmed` / `_shallow` (type-P only) | Phase 13 Plan 02 (D-10): a lane that never produces a single buffer-state transition never arms the trip, regardless of dwell duration |
| `sem_psf_mm_before_ms` / `_shallow` (type-P only) | Phase 13 Plan 02/03 (D-08/D-09): the mm distance trip fires before the ms dwell fallback could ever elapse — proves distance is primary, not incidental ordering |
| `sem_psf_ms_fallback` / `_shallow` (type-P only) | Phase 13 Plan 02/03 (D-08/D-09): with the mm trip disabled (`tension_stop_mm_disabled`), the ms dwell fallback fires on its own on a lane sustaining continuous `BUF_TENSION` for the full dwell window without ever reaching the physical rail |
| `sem_psf_trip_held_suppressed` / `_shallow` (type-P only) | Phase 13 Plan 02 (D-26): a `BL:T` lock armed across the whole window suppresses both the mm and ms trips entirely |
| `sem_psf_trip_hold_release` / `_shallow` (type-P only) | Phase 13 Plan 02 (REVIEW-04): a lock releasing mid-episode resets the accumulator and arm flag — nothing trips in the post-release window even though the pin continues |
| `sem_psf_probe_no_consumer` (type-P only) | Phase 13 Plan 03 (D-15/D-18): a total jam (`feed_gain=0`) — the type-P feed probe emits `PROBE:NO_CONSUMER` exactly once and short-circuits straight to `FAULT_HOLD`, never reaching `TENSION_STOP:MM`. Forces the lane IN sensor clear for a settle window first (a scenario-only technique, not firmware behavior) so the probe's window opens on an already-converged reading rather than the shallow zone-crossing tick of a still-falling dive — see the scenario's own comment for why a straightforward construction manufactures a false `CONSUMER` instead |
| `sem_psf_probe_consumer` / `sem_psf_probe_slow_consumer` (type-P only) | Phase 13 Plan 03 (D-15/D-18, REVIEW-02): a chronic 0.7x-underfeed lane (13-02's `sem_psf_mm_trip` recipe) probes `CONSUMER` — the buffer measurably creeps off its deepest reading — and the mm trip still fires afterward unaffected. `slow_consumer` is the same recipe re-asserted against the REVIEW-02 window-max latch specifically: the boundary-tick instantaneous reading is already back deep by decision time, so only the window-max (not a point-in-time sample) resolves `CONSUMER` correctly |
| `sem_psf_probe_big_buffer` (type-P only) | Phase 13 Plan 03 (REVIEW-03): `buf_max_travel_override` (64mm) set above the `SYNC_TENSION_STOP_MM` default (32mm) — the probe still resolves at its clamped threshold and its `PROBE:` row precedes `TENSION_STOP:MM` in the event stream |
