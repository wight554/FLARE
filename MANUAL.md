...
| `BL[:<T\|C>[:<follow_mm>:<follow_rate_mmpm>[:<timeout_ms>]]]` | OK | **Buffer Lock** — arm the active lane to drive the buffer to the requested extreme at `SYNC_MAX_SPS` and hold there. `BL:T` (tension, default) or `BL:C` (compression). The prime move is capped at full `BUF_MAX_TRAVEL_MM`; once at the extreme the lane holds with motor energized. Bare `BL:T`/`BL:C` is a passive lock: it holds until `BS` or the watchdog and never breaks or catches. With `follow_mm` and `follow_rate_mmpm` supplied, external force (printer retract) breaks the lock (`EV:BL:BREAK`) and the rate-servo catch engages (`EV:BL:FOLLOW`) for that distance/rate; optional trailing `timeout_ms` overrides the default 30000ms watchdog for that arm only. Cancels active buffer stabilize or sync before arming; returns `ER:BUSY:BL` for hard activities or unrelated lane tasks. Use `BS` to release manually. |
| `SM:<0\|1>`| OK | **Sync Mode** — manually toggle buffer sync. |
| `PROBE:` | OK | **Force Type-P Feed Probe** — bench/diagnostic aid: forces the type-P feed probe (Phase 13 SC#3) to restart its window immediately, resetting the shared `SYNC_TENSION_STOP_MM` accumulator and the once-per-episode latch. The probe otherwise fires automatically once per pinned episode; this just makes the next probe distance of pinned feed measured from now, for bench testing without waiting for an organic pin. Returns `ER:NOT_TYPE_P` if the buffer sensor isn't analog, `ER:SYNC_NOT_ACTIVE` if sync isn't engaged, or `ER:HOLD_ACTIVE` if a deliberate rail hold (`BL:`, tail-assist, buffer-stabilize, RELOAD follow) is in progress. |
...
| `SYNC_PSF_RELIEF_MULT` | `sync_psf_relief_mult` | Type-P bounded-relief multiplier applied to the estimated extruder demand while the buffer is starved into the tension rail (Phase 13 SC#1). The bound is floored at the learned per-lane flow-schedule baseline and ceilinged at the clamped `SYNC_MAX_RATE`. Live and flash-persisted (unlike its `SYNC_PSF_*` siblings above). Clamped 1.0-3.0. | 1.33 |
...
| Parameter | `config.ini` Key | Description | Default |
|-----------|------------------|-------------|---------|
| `SYNC_KP_RATE` | `sync_kp_rate` | Proportional reserve-correction window around the virtual buffer target | 2025 |
...
| `SYNC` | `AUTO_START\|AUTO_STOP\|FAULT_HOLD\|FAULT_HOLD_RECOVERY\|TENSION_DWELL_WARN\|TENSION_RISK_HIGH\|RELIEF_PAUSE\|RELIEF_ON\|RELIEF_OFF\|NEUTRAL_CREEP_CAP\|TENSION_STOP:MM\|TENSION_STOP:MS\|PROBE:CONSUMER\|PROBE:NO_CONSUMER\|cannot_refill\|cannot_relieve` | Automatic sync state transitions and warnings. `FAULT_HOLD` fires on tension-dwell timeout; recovers automatically. `TENSION_DWELL_WARN` fires when centering drift reaches a significant threshold. `TENSION_RISK_HIGH` fires when tension-pin density reaches `TENSION_RISK_THR`. `RELIEF_PAUSE` indicates a temporary relief pause; `RELIEF_ON`/`RELIEF_OFF` bracket a type-P bounded-relief episode (Phase 13 SC#1) on entry/exit edges only, never per tick, and are distinct from `RELIEF_PAUSE`; `NEUTRAL_CREEP_CAP` fires when creep velocity hits its cap; `TENSION_STOP:MM`/`TENSION_STOP:MS` (Phase 13 SC#2) fire immediately before the `FAULT_HOLD` they cause, naming which trip won — `TENSION_STOP:MM` is the primary distance-based trip (`SYNC_TENSION_STOP_MM`), `TENSION_STOP:MS` is the slow-flow fallback (`SYNC_TENSION_STOP_MS`), and both share one arm/hold-suppression gate so neither can fire until the first buffer-state transition after re-entry and neither fires during a deliberate rail hold; `PROBE:CONSUMER`/`PROBE:NO_CONSUMER` (Phase 13 SC#3) fire once per pinned episode after one probe distance of pinned relief feed — `PROBE:CONSUMER` means relief is working (a real consumer moved the buffer measurably off its deepest reading) and the mm/ms trips keep running unaffected; `PROBE:NO_CONSUMER` means the buffer never moved and short-circuits straight into the same escalate-then-fault-hold path the trips use, ahead of either trip's own threshold; `cannot_refill` and `cannot_relieve` fire when sync limits are exceeded — on type-P at the default 32 mm `SYNC_TENSION_STOP_MM`, the mm trip fires and resets the shared accumulator before `cannot_refill`'s 50 mm warn threshold can ever be reached, so that warn is practically dormant for type-P and remains primarily a type-D diagnostic (raise `SYNC_TENSION_STOP_MM` above 50, or set it to 0, to make the warn reachable again). |
...
| `BL` | `PRIME\|LOCKED\|BREAK\|FOLLOW\|FOLLOW_DONE\|FOLLOW_GATED\|PRIME_BOUND\|TIMEOUT` | Buffer-lock sequence events. `PRIME` on prime start; `LOCKED` on successful lock; `BREAK` when external force pulls an explicit-follow lock off its extreme; `FOLLOW` on following extruder; `FOLLOW_DONE` on follow finish; `FOLLOW_GATED` on early position gate; `PRIME_BOUND` on travel boundary; `TIMEOUT` on watchdog timeout. |
