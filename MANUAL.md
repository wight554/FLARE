...
| `SYNC_TENSION_PROBE_NEUTRAL` | `sync_tension_probe_neutral` | Runtime-only type-D recovery floor uncertainty-creep rate in mm/min per s, applied each tick while `BUF_NEUTRAL`. Creeps the floor up toward the safe COMPRESSION rail so metastable `feed == demand` resolves into a compression click instead of drifting into TENSION. `0` = hold (no creep). Clamped 0 to 12000. | 300 |
