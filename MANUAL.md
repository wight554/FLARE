...
| `SYNC_TENSION_PROBE_DOWN` | `sync_tension_probe_down` | Runtime-only type-D recovery floor ease-off rate in mm/min per s, applied each tick while `BUF_COMPRESSION` (overfed). COMPRESSION, not a timeout, ends recovery. Clamped 0 to 12000. | 600 |
