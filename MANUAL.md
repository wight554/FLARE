...
3. Capture data using the observe-only tuner in daemon mode, emitting CSV:
   ...
   (**DEPRECATED**: shell-marker input injects per-layer G-code markers that
   block Klipper's queue and cause print lag; prefer sidecar capture).
...
- `--klipper-mode auto|on|off`: auto-fallback, require UDS, or force marker fallback (**DEPRECATED**).
- `--sidecar PATH`: sidecar JSON generated by `gcode_marker.py --emit sidecar`.
- `--marker-file PATH`: tail local marker file written by `scripts/flare_marker.py` (**DEPRECATED**).
- `--keep-marker-file`: debug/attach-only; preserve existing `--marker-file`
  contents (**DEPRECATED** with `--marker-file`).
