...
| `firmware/src/sync.c` | Sync orchestration, buffer-lock state, boot stabilization |
| `firmware/src/sync_buf.c` | Buffer sensing, virtual position, Type-D/Type-P signal publishing, estimator updates |
| `firmware/src/sync_relay.c` | Type-D relay control, neutral feed sampling, relay trim |
| `firmware/src/sync_analog.c` | Type-P analog sync control helpers, reserve metrics, PSF control law |
| `firmware/src/toolchange.c` | Cutter sequencing, toolchange flow, RELOAD orchestration |
| `firmware/src/protocol.c` | USB serial command parsing, motion/system commands, SET/GET |
| `firmware/src/protocol_status.c` | `ST:` status dump formatting |
| `firmware/src/protocol_tmc.c` | Advanced TMC serial commands (`CA:`, `TW:`, `TR:`, `RR:`) |
...
| `firmware/include/protocol_internal.h` | Shared protocol split-unit declarations |
| `firmware/include/sync_internal.h` | Shared sync split-unit declarations |
| `firmware/include/buf_signal.h` | Buffer signal source/types shared by sync modules |
