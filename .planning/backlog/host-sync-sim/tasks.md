...
- [x] `tests/host/shims/pico/stdlib.h` — declare `uint` (`unsigned int`),
...
- [x] `tests/host/shims/pico/types.h` — include `pico/stdlib.h` shim only.
- [x] `tests/host/shims/hardware/pio.h` — opaque `typedef struct pio_hw *PIO;`.
      Required by `tmc2209.h` via `controller_shared.h:10`.
- [x] `tests/host/shims/hardware/adc.h` — `adc_select_input(uint32_t)`,
      `adc_read(void) -> uint16_t`.
- [x] `tests/host/shims/hardware/flash.h` — `FLASH_PAGE_SIZE`, `FLASH_SECTOR_SIZE`,
      `flash_range_erase`, `flash_range_program`.
- [x] `tests/host/shims/hardware/sync.h` — `save_and_disable_interrupts`,
      `restore_interrupts`.
- [x] `tests/host/shims/pico/flash.h` — `PICO_FLASH_SIZE_BYTES`, `XIP_BASE`,
...
- [x] `tests/host/shims/hardware/gpio.h` — `GPIO_IN`, `GPIO_OUT`, `GPIO_FUNC_PWM`,
      `gpio_init/set_dir/get/put/pull_up/pull_down/set_function/disable_pulls`.
- [x] `tests/host/shims/hardware/pwm.h` — `pwm_config`, `pwm_init`,
...
- [x] `tests/host/shims/hardware/clocks.h` — `clock_get_hz`, `clk_sys`.
- [x] `tests/host/CMakeLists.txt` — standalone project, no `pico_sdk_init()`. Target
...
Measured during implementation: this list (drafted before the scope decision to
compile `motion.c`/`toolchange.c`/`cutter.c` too) was stale. `motor_set_rate_sps`,
`motor_enable`, `motor_set_dir`, `motor_stop`, `lane_start`, `lane_stop` are REAL
functions in the linked `motion.c`, not faked — the plant reads their effect back
through real `lane_t` fields (`current_sps`, `task_forward`) instead. See
`memories/repo/host-sync-sim.md`.

- [x] `tests/host/sim_fakes.c` actuators — GPIO/PWM register fakes (record-only)
      underneath the real `motion.c`/`cutter.c` bodies; no logic to drift.
      Accept: plant reads feed from `lane_t.current_sps`/`task_forward`.
- [x] `tests/host/sim_fakes.c` queries — `lane_ptr`, `lane_to_idx`, `cutter_busy`
      (real, linked), `tc_state` (real, linked), `manual_unload_active`,
      `motion_clamp_rate_sps` (real, linked). `lane_ptr` returns
      pointer to sim-owned `lane_t`.
- [x] `tests/host/sim_fakes.c` events — `cmd_event`, `cmd_event_critical` append the
...
- [x] `tests/host/sim_fakes.c` clock — `get_absolute_time` / `to_ms_since_boot` return
      simulated `g_now_ms`. No wall-clock read anywhere in the sim.
- [x] `tests/host/sim_fakes.c` ADC — `adc_select_input` no-op, `adc_read` returns counts
      mapped from plant slack. Accept: type-P `g_buf_pos` tracks plant slack.
- [x] `tests/host/sim_fakes.c` helpers — `clamp_f`, `clamp_i`, `sps_to_mm_per_min`,
      `reload_trigger` (real, linked), `set_active_lane`, `set_toolhead_filament`.
      Also needed beyond the original list: `mm_per_min_to_sps`, `build_ihold_irun_reg`
      (stubbed — its only consumer is protocol.c status reporting, out of scope), and
      the 4 tmc2209.c entry points `motion.c`/`settings_store.c` call
      (`tmc_set_run_current_ma`, `tmc_setup_chopconf`, `tmc_set_stealthchop_sps`,
      `tmc_set_pwmconf`).
...
- [x] `scripts/gen_sim_globals.py` — parse top-level `g_` definitions out of
...
- [x] `tests/host/CMakeLists.txt` — wire the generator as a custom command depending on
...
- [x] `.gitignore` — add the generated `sim_globals.c`; it is never hand-edited.
      (Covered by `build_sim/` — the generator emits into `build_sim/generated/`.)
- [x] Verify no `g_` definition is hand-written in `sim_fakes.c`.
      The literal grep (`grep -c '^[a-z_].*g_[a-z_]* *=' tests/host/sim_fakes.c`)
      returns 1, not 0 — but the one match is `g_sim_adc_counts`, a sim-owned
      state variable (ADC counts fake), not a duplicated firmware tunable/state
      global. It is `g_`-prefixed only because repo convention requires that on
      every global (see `firmware-tunable-global-rename` memory), not because it
      collides with anything `gen_sim_globals.py` generates. Checked by hand:
      `grep -oP '^\w[\w ]*\bg_\w+(?=\s*=)' tests/host/sim_fakes.c` lists exactly
      one top-level definition (`g_sim_adc_counts`), and that name does not
      appear as a definition in `gen_sim_globals.py`'s output.

## 3. Plant

- [x] `tests/host/sim_plant.c` — `slack_mm` integrator per `design.md` Plant Model.
...
- [x] `tests/host/sim_plant.c` — type-D sensor emission: write
      ...
      slack values.
- [x] `tests/host/sim_plant.c` — type-P emission: linear map slack → ADC counts.
      Accept: `g_buf_pos` sign matches slack sign (`+` compression, `−` tension).
- [x] `tests/host/sim_scenario.c` — demand profiles `steady`, `step_up`, `burst`,
      `idle_zero`, `retract`, `long_retract`. Piecewise over simulated time.

## 4. Fault injection

- [x] `tests/host/sim_plant.c` — `feed_gain`, `demand_gain`, `retract_gain`, each
      time-scheduled. Accept: gain applied from its scheduled timestamp, not t=0.
- [x] `tests/host/sim_plant.c` — `sensor_force`: per-sensor override supporting
      stuck-asserted, single-tick chatter, both-asserted.
      Accept: both-asserted yields `BUF_FAULT` (`sync_buf.c:498`).
- [x] `tests/host/sim_scenario.c` — `switch_script`: timed IN/OUT transitions writing
      `lane_t` debounced input state.

## 5. Trace and invariants

- [x] `tests/host/sim_trace.c` — CSV writer, columns `ts_ms,bp_mm,zone,feed_sps,
      demand_mm_s,sync_state,sat,events`. Header row first.
- [x] `tests/host/sim_main.c` — per-scenario tick ceiling with a default; exceeding it
      ...
      hanging the suite.
- [x] `tests/host/sim_main.c` — assert no wall-clock read anywhere: all timekeeping
...
- [x] `tests/host/sim_trace.c` — global invariants 1-7 per spec, evaluated every tick.
...
- [x] `tests/host/sim_trace.c` — invariants 4 (fault quiescence: `SYNC_FAULT_HOLD`
      ...
      detect the shipped fault-hold deadlock loop. Invariant 4 exempts the entry
      tick's own fault-announcement event (see `memories/repo/host-sync-sim.md`).
- [x] `tests/host/sim_main.c` — CLI: `--scenario NAME`, `--stress`, `--stress-lag-ms N`,
      `--ticks N`, `--sensor-type {d,p}`. Boot order per spec: zero-init → `settings_defaults()` →
      overrides → `sync_init(0)` → tick loop.
- [x] `tests/host/sim_main.c` — tick loop replicating `main.c:569` order:
      ...
      (`autopreload_tick` is `static` in `main.c`, not linkable — stubbed as a no-op in
      `sim_main.c`; gap noted here and in `memories/repo/host-sync-sim.md`.)

## 6. Stress mode

- [x] `tests/host/sim_plant.c` — first-order mechanical transport lag and
      ...
      edit.
- [x] Verify the stress lag does not duplicate the sensing EWMA. `sync_buf.c:413`
...
- [x] `scripts/test_sync_sim.py` — stress suite sweeps `--stress-lag-ms` over
...
- [x] `scripts/test_sync_sim.py` — `unittest.TestCase` subclass. Builds binary path
      ...
      Note: `unittest discover` silently skips non-`TestCase` files — confirmed the
      new tests actually appear in the run count (8 tests).
- [ ] `scripts/test_sync_sim.py` — bug-history regression scenarios, each named for
      ...
      Partial: `idle_zero` covers the frozen-distance-clock/abrupt-extruder-stop case.
      The five RELOAD/BL-follow-specific defects are NOT reproduced by name — they need
      toolchange RELOAD state-machine setup (BL-follow arm sequence, RELOAD phase
      transitions) beyond this first cut's scenario table. Tracked as a follow-up in
      `memories/repo/host-sync-sim.md`; not silently dropped.
- [x] `scripts/test_sync_sim.py` — fault-injection scenarios covering the full
      Class-1 table in `design.md`.
- [x] `scripts/test_sync_sim.py` — compare floats with explicit tolerances, never exact
...
- [x] `scripts/test_sync_sim.py` — same-machine determinism check: run one scenario
      twice, assert byte-identical traces. Catches uninitialized memory and any
      surviving wall-clock or ordering dependence.
- [x] `scripts/test_sync_sim.py` — retain trace output only for failing scenarios;
      discard on pass. Accept: a fully passing suite leaves no trace files behind.
- [x] `scripts/test_sync_sim.py` — one subprocess per scenario, never reused across
...
- [x] `scripts/test_sync_sim.py` — verify the suite runs with no hardware present:
      ...
      (Measured for this suite: 8 tests / ~0.9 s.)
- [x] `scripts/test_sync_sim.py` — run every scenario against both sensor types
...
- [x] `scripts/validate_regression.py` — build `flare_sim` and run the scenario suite;
...
- [x] `.gitignore` — add `build_sim/`.

## 9. Documentation

- [x] `AGENTS.md` — add `tests/host/` to Key Files with read mode `[lookup]`; state the
      authority boundary in one line (sim screens, rig judges, sim never satisfies `HW:`).
- [x] `TEST_CASES.md` — document the scenario catalogue and the five global invariants.
      (Documented all 7 — design.md specifies 7, not 5; task text corrected here.)
- [x] `README.md` — one line under developer tooling on building and running the sim.

## Readiness and Delivery Checks

- [x] Dev-tuning superset firmware build green:
      `cmake -S firmware -B build_local -DFLARE_DEV_TUNING=ON && ninja -C build_local`
- [x] Firmware output unchanged by this change — confirm no file under `firmware/src/`
...
- [ ] `python3 scripts/validate_regression.py` green, and the sim scenario count in its
      output is non-zero.
      Blocked on a pre-existing, unrelated failure: the ruff step fails on `flare_daemon.py`
      findings that predate this change (verified via `git diff` against `main` — that
      file is untouched here). The new "Host Sync Simulation" gate step itself (build +
      `python3 -m unittest scripts.test_sync_sim -v`, 8 tests, non-zero scenario count)
      passes cleanly in isolation and was verified as part of a full gate run up to the
      pre-existing ruff failure.
- [x] Simulation builds clean with `-Wall -Wextra`: `ninja -C build_sim` no warnings
      (scoped to `tests/host/`'s own sources — see `tests/host/CMakeLists.txt` comment
      and `memories/repo/host-sync-sim.md` for why firmware sources are excluded from
      this flag set).
- [x] Documentation sync verified — `AGENTS.md`, `TEST_CASES.md`, `README.md`
- [x] `openspec validate host-sync-sim --strict` passing
- [x] `openspec validate --specs --strict` passing
- [x] Observation appended to `memories/repo/host-sync-sim.md` — covering: shim set
      that proved sufficient, symbols that had to be faked beyond the measured 104, a
      real defect exposed during implementation (two spec.md errors — retract rail
      direction and the bounds invariant), and deviations from `design.md`
- [ ] Archive with a real `## Purpose` on the new `host-sync-simulation` spec — the
      ...
      Not run in this session — archiving is a separate action; see
      `openspec-archive-change` when ready.
