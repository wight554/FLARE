...
DUMP_PARAMS = [
    ...
    ("PSF_STAB_STAGNANT_MS", "psf_stab_stagnant_ms", False),
    ("PSF_STAB_STAGNANT_NORM", "psf_stab_stagnant_norm", False),
    ("PSF_STAB_RAIL_BREAK_MS", "psf_stab_rail_break_ms", False),
...
def run_dump_daemon(args):
    ...
    for (cmd, key, lane_aware) in DUMP_PARAMS:
        if lane_aware:
            vals = []
            for suffix, _label in [("_L1", "L1"), ("_L2", "L2")]:
    ...
    if args.raw:
        ...
        print("# Generated by flare_cmd.py --dump")
        print()
        for (_, key, _) in DUMP_PARAMS:
