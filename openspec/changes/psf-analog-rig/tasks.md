...
2026-05-30 RIG FIX 7: Boot/idle stabilization fought locked extruder gears on boot/idle when already homed or when filament was blocked. Root cause: boot/idle stabilization started unconditionally on boot if filament was present, even if the buffer was already at its homed state. If filament was gripped by locked extruder gears, the motor would continuously drive forward/backward without the buffer position changing, risking grinding or motor damage. Fix 1: gate boot/idle stabilization for Type-P to only start when buf_state_raw() != BUF_HOME_STATE. Fix 2: implement a 500 ms stagnant timeout check in buffer_stabilize_tick() for Type-P; if g_buf_pos moves less than 0.05f (5% range) after 500 ms, stop the motor immediately and exit neutralization. Ninja clean. Commit `10823d8`.
