...
- **NEW** `scripts/flare_mmu.cfg` — single-include Klipper config covering:
  ...
  - `[gcode_macro _FLARE_LOAD_HOTEND]` — 3-stage meltzone approach + optional purge after TC:
...
- **MODIFIED** `KLIPPER.md` toolchange macros section: points to `flare_mmu.cfg` instead of inline snippets

## Capabilities

### New Capabilities

- `klipper-mmu-config`: Single-file Klipper MMU integration with tip forming, load sequence, toolchange, and boot-time RELOAD_MODE configuration
