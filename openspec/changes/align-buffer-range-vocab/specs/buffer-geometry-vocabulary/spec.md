...
- `buf_sense_span_mm` — the FULL distance the filament travels between the
...
- **THEN** `buf_sense_span_mm` and `buf_max_travel_mm` are represented in
  `config.ini`
...
The firmware SHALL convert the full-range `buf_sense_span_mm` to the
internal half-based geometry exactly once at the value-ingest boundary
(config ingest and the serial SET handler) as
`half = buf_sense_span_mm / 2`, and SHALL NOT apply the conversion anywhere
...
- **WHEN** `buf_sense_span_mm = 10` is ingested
...
`buf_sense_span_mm = 10` and `buf_max_travel_mm = 25`. These replace the
...
- **THEN** `buf_sense_span_mm` defaults to `10` (internal half `5`)
- **AND** `buf_max_travel_mm` defaults to `25`

### Requirement: Sense-span and max-travel clamp relationship
The two tunables SHALL preserve, in full-range terms, the clamp relationship
that existed in half-range terms. `buf_sense_span_mm` SHALL be clamped to
`[2.0, buf_max_travel_mm]`. `buf_max_travel_mm` SHALL be clamped to
`[10, 1000]`. Setting `buf_max_travel_mm` SHALL re-clamp `buf_sense_span_mm`
so the derived internal half never exceeds `buf_max_travel_mm / 2`.

#### Scenario: Sense-span clamped against max-travel
- **WHEN** a SET requests `buf_sense_span_mm` greater than the current
  `buf_max_travel_mm`
- **THEN** `buf_sense_span_mm` is clamped to `buf_max_travel_mm`

#### Scenario: Lowering max-travel re-clamps sense-span
- **WHEN** `buf_max_travel_mm` is set below the current `buf_sense_span_mm`
- **THEN** `buf_sense_span_mm` is reduced so the internal half stays
  `≤ buf_max_travel_mm / 2`

### Requirement: Serial vocabulary rename without aliases
The serial SET/GET tokens SHALL be `BUF_SENSE_SPAN` and `BUF_MAX_TRAVEL`,
...
- **WHEN** `SET:BUF_SENSE_SPAN:10` is issued and `GET BUF_SENSE_SPAN` is read
...
- **WHEN** type-P firmware runs with `buf_sense_span_mm = 10`,
