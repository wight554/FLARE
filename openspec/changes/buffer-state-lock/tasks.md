...
- [ ] 9.10 Bench `sync_ramp_accel = 150 mm/s²` on rig: run a long sync soak with mixed flow (low + high extrusion rate, both directions). Watch for buffer ringing around `BUF_NEUTRAL` (>1 Hz switch toggle = unstable). If unstable, apply `sync_kp_rate ← sync_kp_rate / k` where `k = sync_ramp_accel / 33` (k ≈ 4.5 at default 150).
