...
- [ ] 1.1 Remove stale file `scripts/test_phase_2_10_parity.py`
- [ ] 1.2 Modify `scripts/test_flare_mmu_status.py` to wrap the `sys.exit` code in an `if __name__ == '__main__'` block
- [ ] 1.3 Update `scripts/validate_regression.sh` to run the full set of unit tests using Python's standard unittest discovery mode (`python3 -m unittest discover -s scripts -p "test_*.py"`)
- [ ] 1.4 Run `scripts/validate_regression.sh` and verify all tests pass cleanly
