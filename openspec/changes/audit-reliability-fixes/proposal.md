...
- `toolchange-orchestration`: type-P RELOAD contact MUST be compression-side (parity with `BUF_COMPRESSION`); type-P follow success MUST preempt the tension refill on the zone-edge trigger, gated past the touch-settle window (no deeper-position requirement — unreachable against `JOIN_SPS` refill); `RELOAD_Y_TIMEOUT_MS=0` MUST disable the Y gate without insta-failing tail-clear waits.
