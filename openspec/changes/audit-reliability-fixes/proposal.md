...
- **H4** (type-P hardware validation of H1, 2026-06-18) `toolchange.c` RELOAD follow: success-on-`BUF_TENSION` and the compression-jam timeout both assume a *consumer* — an extruder drawing the old tail. With an idle extruder (paused print, manual `RL:` retrigger after the print stopped, or bench) the buffer pegs `+1.0` COMPRESSION and can never reach TENSION, so the "stuck in compression" timeout false-fires `FOLLOW_JAM`. The sensor reads correctly (not a sign bug); the completion model is incomplete. Fix: fork follow on `g_extruder_est_sps` vs `RELOAD_CONSUMER_MIN_SPS` — no consumer → completion = sustained COMPRESSION contact past the touch-settle window (new filament staged at the extruder mouth), compression-jam detectors suppressed, absolute timeout retained. Companion: `RL:` was a fixed same-lane routine (`tc_manual_reload`) divergent from the auto path (`reload_trigger`); make it a state-aware resume — delegate to `reload_trigger` (swap) when the active lane ran out and the other is loaded, else resume approach/follow on the active lane.
...
- `toolchange-orchestration`: type-P RELOAD contact MUST be compression-side (parity with `BUF_COMPRESSION`); type-P follow success MUST preempt the tension refill on the zone-edge trigger, gated past the touch-settle window (no deeper-position requirement — unreachable against `JOIN_SPS` refill); `RELOAD_Y_TIMEOUT_MS=0` MUST disable the Y gate without insta-failing tail-clear waits; RELOAD follow MUST fork on consumer presence (no-consumer completion = staged compression, jam detectors suppressed); manual `RL:` MUST be a state-aware resume of the auto-runout sequence (H4).
...
- Hardware validation needed: H1/H4 on a type-P rig (runout RELOAD + `RL:` — incl. paused/no-consumer staged completion and the missed-swap resume), H2 dry-spin re-fire, L1 prime ramp at raised SYNC_MAX. H3/M1 verifiable by bench + build.
