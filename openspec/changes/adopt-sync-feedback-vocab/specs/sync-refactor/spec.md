...
Live prose SHALL name the sensor (SFS/DSF/PSF) separately from the control
law. The discrete path's law SHALL be referred to as the "DSF two-level /
hysteretic relay control law" and the analog path's law as "PSF PD/EKF
(reserve) control law"; "2-switch" and "relay" MUST NOT be used as if they
named the sensor.

#### Scenario: Discrete control law named without conflation

- **WHEN** a live spec or doc describes the discrete-path controller
- **THEN** it names the DSF sensor and the two-level/relay law as distinct
  layers, not a single "2-switch relay" sensor-feature
