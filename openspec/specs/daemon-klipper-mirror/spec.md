...
The daemon SHALL emit a full `SET_MMU` (all fields) on the first push, on the periodic
force-resync interval, and on a board-online transition, so a restarted Klipper/Moonraker
recovers complete state.

#### Scenario: Klipper restarts mid-print

- **WHEN** the force-resync interval elapses or the board comes online
