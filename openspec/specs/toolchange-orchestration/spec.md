...
- **THEN** the system executes `UNLOAD_CUT` (if `UNLOAD_CUT` is enabled), `UNLOAD_REVERSE`, waits for `OUT` to clear, waits for Y-splitter clear, updates `active_lane`, and starts `LOAD_FULL`
