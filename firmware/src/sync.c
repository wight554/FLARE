...
#define SYNC_COMPRESSION_COLLAPSE_DELAY_MS ((uint32_t)CONF_RELAY_COLLAPSE_DELAY_MS)
#define SYNC_COMPRESSION_COLLAPSE_RAMP_MULT CONF_RELAY_COLLAPSE_RAMP_MULT
#define SYNC_COMPRESSION_COLLAPSE_CAP_MS ((uint32_t)CONF_RELAY_COLLAPSE_CAP_MS)
...
static void sync_on_transition(buf_state_t prev, buf_state_t now_state, uint32_t now_ms) {
    ...
    if (prev != BUF_NEUTRAL && now_state == BUF_NEUTRAL && sync_enabled &&
        !fast_brake_active && !sync_compression_recovery_active && !g_sync_hold) {
